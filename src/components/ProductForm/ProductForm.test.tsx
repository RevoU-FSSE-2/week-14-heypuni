...
describe('Test List of Category Component', () => {
    ...
    it('Name Title & Input Renders Correctly',async () => {
        const { getByPlaceholderText } = render(<BrowserRouter><ProductForm onSubmit={mockProps.onSubmit} content={productContent.content} /></BrowserRouter>)
    ...
    it('Status Title & Select Renders Correctly',async () => {
        render(<BrowserRouter><ProductForm onSubmit={mockProps.onSubmit} content={productContent.content} /></BrowserRouter>)
    ...
    it('Back/Return Link Renders Correctly', async () => {
        const { getByText } = render(<BrowserRouter><ProductForm onSubmit={mockProps.onSubmit} content={productContent.content} /></BrowserRouter>)
    ...
    it('Test onSubmit (Select Option: "Active") Works Correctly', async () => {
        const { getByPlaceholderText, getByText } = render(<BrowserRouter><ProductForm onSubmit={mockProps.onSubmit} content={productContent.content} /></BrowserRouter>)
    ...
    it('Test onSubmit (Select Option: "Deactive") Works Correctly', async () => {
      ...
      await waitFor(()=> {
          expect(mockProps.onSubmit).toHaveBeenCalledTimes(2);
          expect(mockProps.onSubmit).toHaveBeenCalledWith({
              name: 'testcreatename',
              is_active: 'false',
          });
